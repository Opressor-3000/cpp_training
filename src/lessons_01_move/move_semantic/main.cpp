/*
Какую перегрузку выберет компилятор?
Цель: 
    Почувствовать разницу между lvalue и rvalue.
Задание: 
    Запусти и подпиши в комментариях, какая строка что выводит и почему (категория выражения).
Ожидаемо: 
    литералы/результаты выражений → &&; 
    именованные переменные → const&; std::move(x) → &&.
*/
#include <iostream>
#include <utility>
